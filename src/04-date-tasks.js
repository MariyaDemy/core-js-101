...
function timeSpanToString(startDate, endDate) {
  ...
  const isoFormat = diffDate.toISOString();
  const timespan = isoFormat.split('T')[1];
  return timespan.slice(0, timespan.length - 1);
}
...
function angleBetweenClockHands(date) {
  ...
  const hoursAngle = 0.5 * (60 * (hour % 12) + minutes);
  const minutesAngle = 6 * minutes;

  let resultAngle = Math.abs(hoursAngle - minutesAngle);
  if (resultAngle > 180) {
